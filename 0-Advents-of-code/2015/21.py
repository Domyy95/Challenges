...
# Naive solution
def battle(player_damage, player_armor):
...
# Better solution considering that the damages are constant
def battle_pro(player_damage, player_armor):
    # True if player wins, False if boss wins
    return math.ceil(
        player_hit_points / max(boss_damage - player_armor, 1)
    ) >= math.ceil(boss_hit_points / max(player_damage - boss_armor, 1))
    ...
    for armor in armors:
        for ring1 in rings:
            for ring2 in rings:
                ...
                if battle_pro(player_damage, player_armor):
                    min_cost = min(min_cost, cost)
                else:
                    max_cost = max(max_cost, cost)

print("Solution 1:", min_cost)
print("Solution 2:", max_cost)
