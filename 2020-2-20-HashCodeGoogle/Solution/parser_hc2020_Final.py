...
if __name__ == '__main__':
    ...
    data.libraries.sort(key=lambda l: sum(list(map(lambda b: b.value, l.books))) / l.signup_days, reverse=True)
    selected_books_set = set()
    i = 0

    while i < data.days and len(data.libraries) != 0:
        data.libraries.sort(key=lambda l: sum(list(map(lambda b: b.value, l.books[0: (data.days - i) * l.books_per_day]))), reverse=True)
        ...
        for t in range(len(data.libraries)):
            data.libraries[t].books = list(filter(lambda b: b.id not in selected_books_set,  data.libraries[t].books))
            data.libraries[t].books = sorted(data.libraries[t].books, key=lambda b: b.value, reverse=True)
