...
class Problem:
    def __init__(self, file_content):
        ...
        self.num_buildings, self.num_antennas, self.reward = (
            int(second_line[0]),
            int(second_line[1]),
            int(second_line[2]),
        )
    ...
    def solve(self, problem):
        ...
        antennas_sorted_per_building = (
            df_pair.groupby(by="id_a").mean().sort_values(by="b_connection")
        )
        antennas_sorted_per_antenna = sorted(
            self.antennas, key=lambda x: x.connection_speed
        )
        antennas_solutions = {}
        for id_a in antennas_sorted_per_building.index:
            x_a, y_a = (
                antennas_sorted_per_building.loc[id_a]["x_a"],
                antennas_sorted_per_building.loc[id_a]["y_a"],
            )
            buildings = df_pair.loc[df_pair["id_a"] == id_a].index
            id_antenna_real = antennas_sorted_per_antenna[id_a].id
            antennas_solutions[id_antenna_real] = (
                x_a,
                y_a,
            )  # (buildings, centroids[id_a])
