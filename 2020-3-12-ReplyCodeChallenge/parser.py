    ...
    def get_output_file(self):
        ...
        for index_sdev in range(0, self.number_dev):
            if index_sdev in list(self.selected_developers):
                output += "{} {}\n".format(
                    self.selected_developers[index_sdev][0],
                    self.selected_developers[index_sdev][1],
                )
            else:
                output += "X\n"
        for index_spm in range(0, self.number_pm):
            if index_spm in list(self.selected_project_managers):
                output += "{} {}\n".format(
                    self.selected_project_managers[index_spm][0],
                    self.selected_project_managers[index_spm][1],
                )
            else:
                output += "X\n"
            ...
            if i == 0:
                ...
                if number_developers == 100001:
                    ...
                    developers[developer_id] = Developer(
                        ...
                        values[0],
                        int(values[1]),
                        skills=set(values[3 : len(values)]),
                    )
                    developer_id += 1
            else:
                if number_project_managers == 0:
                    ...
                    values = line.split(" ")
                    project_managers[project_manager_id] = ProjectManager(
                        project_manager_id, values[0], int(values[1])
                    )
...
if __name__ == "__main__":
    ...
    data.developers = [
        v
        for k, v in sorted(
            data.developers.items(), key=lambda item: len(item[1].skills), reverse=True
        )
    ]
    data.project_managers = [
        v
        for k, v in sorted(
            data.project_managers.items(), key=lambda item: item[1].bonus, reverse=True
        )
    ]
    ...
    while score != 0 and len(data.mapOffice) != 0:
        data.mapOffice = sorted(
            data.mapOffice, key=lambda x: len(x.siblings), reverse=True
        )
        ...
        if len(besties_devs) > 0:
            score_dev = len(boi.skills.intersection(besties_devs[0].skills)) * len(
                boi.skills.union(besties_devs[0].skills)
                - boi.skills.intersection(besties_devs[0].skills)
            ) + (
                boi.bonus * besties_devs[0].bonus
                if boi.company == besties_devs[0].company
                else 0
            )
        else:
            score_dev = 0
        ...
        if len(besties_pm) > 0:
            score_pm = (
                boi.bonus * besties_pm[0].bonus
                if boi.company == besties_pm[0].company
                else 0
            )
        else:
            score_pm = 0
